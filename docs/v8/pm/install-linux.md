...
为减少对操作系统的影响，不建议使用 `root` 用户安装和运行 DM，建议提前创建专用系统用户：
...
如果系统中已存在 DM，会在终端提示是否继续；选择继续则进入下一步，否则退出安装。

![是否继续](./images/install-uninstall/图2.40-是否继续.png)

> 若系统中已安装 DM，重新安装前应完全卸载原有版本，并提前备份数据。
...
> DM 提供的各项服务依赖网络和存储才能正常启动，若网络或存储未就绪可能导致启动失败，可等待就绪后重新启动相关服务，或调整服务脚本中的优先级与依赖关系。
...
DM 默认安装目录为 `$HOME/dmdbms`（若以 `root` 用户安装，默认目录为 `/opt/dmdbms`，但不建议使用 `root` 安装）。

> 安装路径只允许使用小写字母（a-z）、大写字母（A-Z）、数字（0-9）、下划线（_）、空格和中文。

![选择安装位置](./images/install-uninstall/图2.32-选择安装位置.jpg)

若目录已存在，会弹出警告，确认后将覆盖该路径下已有的 DM 组件。
